...
$$ AED = 66 d_f (\frac {\beta} {2 + 4\beta})^{2.2} $$
...
with(fiber_data %>% filter(fiber_type == 'Amphibole'), cor.test(log(length), log(width)))
with(fiber_data %>% filter(fiber_type == 'Amphibole'), cor.test(log(length), log(width), method = 'spearman'))
