...
```{r}
a_mod <- lm(log(norm_conc) ~ norm_dist - 1, fiber_conc2 %>% filter(fiber_type == 'Crocidolite'))
...
with(fiber_conc2 %>% filter(fiber_type == 'Crocidolite'), cor.test(log(aed), log(fiber_cc)))
